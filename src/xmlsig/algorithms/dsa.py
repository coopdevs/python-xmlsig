...
from xmlsig.ns import DSigNs, NS_MAP
from xmlsig.utils import create_node, long_to_bytes, b64_print, USING_PYTHON2
...
class DSAAlgorithm(Algorithm):
    ...
    def verify(signature_value, data, public_key, digest):
        public_key.verify(
            DSASignature.from_p1363(b64decode(signature_value)).dump(),
            data,
            digest()
        )
    ...
    def key_value(node, public_key):
        ...
        if public_numbers.parameter_numbers.p is not None:
            create_node(
                ...
                DSigNs,
                tail='\n',
                text='\n' + b64_print(b64encode(long_to_bytes(
    ...
    def get_public_key(key_info, context):
        key = key_info.find(
            'ds:KeyInfo/ds:KeyValue/ds:DSAKeyValue', namespaces=NS_MAP
        )
        if key is not None:
            ...
            return dsa.DSAPublicNumbers(
                y, dsa.DSAParameterNumbers(p, q, g)
            ).public_key(default_backend())
        return super(DSAAlgorithm, DSAAlgorithm).get_public_key(
            key_info, context
        )
